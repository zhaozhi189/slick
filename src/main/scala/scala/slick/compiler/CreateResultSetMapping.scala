...
class CreateResultSetMapping extends Phase {
  ...
  def apply(state: CompilerState) = state.map { n =>
    ...
    ClientSideOp.mapServerSide(n3) { ch =>
      val tables: Map[TypeSymbol, Node] = ch.collect {
        case NodeType(TableExpansion(_, _, columns), CollectionType(_, NominalType(ts))) =>
          ts -> columns
      }.toMap
  ...
  def removeMappedType(tpe: Type): Type = tpe.structural match {
    case StructType(ch) =>
