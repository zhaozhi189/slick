...
import scala.slick.ast.{SimpleTableIdentitySymbol, TableIdentitySymbol, Library, FunctionSymbol, Dump, ColumnOption}
...
class SlickBackend( val driver: JdbcDriver, mapper:Mapper ) extends QueryableBackend{
  ...
  def typetagToQuery(typetag:TypeTag[_]) : Query = {
    ...
    val table = new sq.TableNode with sq.WithOp with sq.TypedNode {
      ...
      val tableIdentitySymbol = SimpleTableIdentitySymbol(driver, schemaName.getOrElse("_"), tableName)
