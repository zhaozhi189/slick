...
final case class TypeMapping(val child: Node, val mapper: MappedScalaType.Mapper) extends UnaryNode with SimplyTypedNode { self =>
  type Self = TypeMapping
  def nodeRebuild(ch: Node) = copy(child = ch)
  override def toString = "TypeMapping"
  protected def buildType = new MappedScalaType(child.nodeType, mapper)
}
