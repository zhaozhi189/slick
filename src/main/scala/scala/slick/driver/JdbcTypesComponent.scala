...
trait JdbcTypesComponent extends RelationalTypesComponent { driver: JdbcDriver =>
  ...
  abstract class MappedJdbcType[T, U](implicit baseType: BaseColumnType[U], tag: ClassTag[T]) extends JdbcType[T] {
    protected[this] lazy val tmd = jdbcTypeFor(baseType).asInstanceOf[JdbcType[U]]
  ...
  protected[this] val jdbcTypes = Map[ScalaBaseType[_], JdbcType[_]]()
  protected[this] def registerType(t: JdbcType[_]): Unit = jdbcTypes += (t.scalaType -> t)

  def jdbcTypeFor(t: Type): JdbcType[Any] = ((t match {
    case tmd: JdbcType[_] => tmd
    case t: OptionType => jdbcTypeFor(t.elementType)
    case t: ScalaBaseType[_] if jdbcTypes.contains(t) => jdbcTypes(t)
  ...
  class CharJdbcType extends DriverJdbcType[Char] {
    protected[this] lazy val stringJdbcType = jdbcTypeFor(ScalaBaseType.stringType).asInstanceOf[JdbcType[String]]
  ...
  trait ImplicitColumnTypes extends super.ImplicitColumnTypes {
    implicit def blobColumnType = JdbcTypesComponent.blobType
    implicit def byteArrayColumnType = JdbcTypesComponent.byteArrayType
    implicit def clobColumnType = JdbcTypesComponent.clobType
    implicit def dateColumnType = JdbcTypesComponent.dateType
    implicit def timeColumnType = JdbcTypesComponent.timeType
    implicit def timestampColumnType = JdbcTypesComponent.timestampType
    implicit def uuidColumnType = JdbcTypesComponent.uuidType
  }

  registerType(new BooleanJdbcType)
  registerType(new BlobJdbcType)
  registerType(new ByteJdbcType)
  registerType(new ByteArrayJdbcType)
  registerType(new CharJdbcType)
  registerType(new ClobJdbcType)
  registerType(new DateJdbcType)
  registerType(new DoubleJdbcType)
  registerType(new FloatJdbcType)
  registerType(new IntJdbcType)
  registerType(new LongJdbcType)
  registerType(new ShortJdbcType)
  ...
  registerType(new BigDecimalJdbcType)
  registerType(new NullJdbcType)
}
...
object JdbcTypesComponent {
  ...
  val blobType = new ScalaBaseType[Blob]
  val byteArrayType = new ScalaBaseType[Array[Byte]]
  val clobType = new ScalaBaseType[Clob]
  val dateType = new ScalaBaseType[Date]
  val timeType = new ScalaBaseType[Time]
  val timestampType = new ScalaBaseType[Timestamp]
  val uuidType = new ScalaBaseType[UUID]
}
